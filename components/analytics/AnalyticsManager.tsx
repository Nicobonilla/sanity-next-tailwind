...
export default function AnalyticsManager({
  ...
  const shouldLoadGa4 =
    trackingEnabled && consent === 'granted' && Boolean(gaMeasurementId);
  const shouldLoadAnalytics = shouldLoadGtm || shouldLoadGa4;
          ...
          {shouldLoadGa4 ? (
            <>
              ...
              <Script
                id="ga4-init"
                strategy="afterInteractive"
              >{`window.dataLayer=window.dataLayer||[];window.gtag=window.gtag||function(){window.dataLayer.push(arguments);};window.gtag('js',new Date());window.gtag('config','${gaMeasurementId}',{send_page_view:false,anonymize_ip:true});`}</Script>
            </>
          ) : null}
          {shouldLoadGtm ? (
            <>

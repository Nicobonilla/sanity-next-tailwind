// components/skeletons/index.tsx
export { default as Banner1Skeleton } from './Banner1Skeleton';
export { default as Banner4ImagesSkeleton } from './Banner4ImagesSkeleton';
export { default as BreadcrumbsSkeleton } from './BreadcrumbsSkeleton';
export { default as CarouselSkeleton } from './BreadcrumbsSkeleton';
export { default as CarouselHeroSkeleton } from './BreadcrumbsSkeleton';
export { default as CarouselPostSkeleton } from './BreadcrumbsSkeleton';
export { default as PortableTextAndTocSkeleton } from './PortableTextAndTocSkeleton';
export { default as PostsSkeleton } from './PostsSkeleton';
export { default as ResourcesSkeleton } from './ResourcesSkeleton';
